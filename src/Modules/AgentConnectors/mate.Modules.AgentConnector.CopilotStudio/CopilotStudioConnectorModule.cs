...
public sealed class CopilotStudioConnectorModule : IAgentConnectorModule
{
    ...
    public IEnumerable<ConfigFieldDefinition> GetConfigDefinition()
    {
        ...
        yield return new ConfigFieldDefinition("WebChannelSecretRef", "Web Channel Security Secret",             "Paste the Web Channel Security secret value, or enter an existing stored secret name. Find it in Copilot Studio → Settings → Security → Web channel security. Required when Use Web Channel Security Secret is true.", "secret",  false);
        yield return new ConfigFieldDefinition("DirectLineSecretRef", "Direct Line Secret",                      "Paste the Direct Line v3 secret value, or enter an existing stored secret name. Required when Use Web Channel Security Secret is false.",                                                                                  "secret",  false);
